...
class GuardarAlarm2Pag extends StatefulWidget {
  static const String ROUTE = '/guardaralarm2';

  @override
  _GuardarAlarm2PagState createState() => _GuardarAlarm2PagState();
}

class _GuardarAlarm2PagState extends State<GuardarAlarm2Pag> {
  ...
  Future<void> _horarioAlarma(
    ...
    switch (cantidaddias) {
      ...
      case 6: // solo los dias marcados
        ...
        print('Cantidad dias: ' + cantidaddias.toString());
        int posicion = scheduledNotificationDateTime.weekday;
        for (var i = 0; i < 7; i++) {
          if (posicion == DateTime.monday && alarmaInfo.lunes == 1) {
            print('lunes ' + DateTime.monday.toString());
            diaActivo = true;
          } else if (posicion == DateTime.tuesday && alarmaInfo.martes == 1) {
            print('Martes ' + DateTime.tuesday.toString());
            diaActivo = true;
          } else if (posicion == DateTime.wednesday &&
              alarmaInfo.miercoles == 1) {
            print('Miercoles ' + DateTime.wednesday.toString());
            diaActivo = true;
          } else if (posicion == DateTime.thursday && alarmaInfo.jueves == 1) {
            print('Jueves ' + DateTime.thursday.toString());
            diaActivo = true;
          } else if (posicion == DateTime.friday && alarmaInfo.viernes == 1) {
            print('Viernes ' + DateTime.friday.toString());
            diaActivo = true;
          } else if (posicion == DateTime.saturday && alarmaInfo.sabado == 1) {
            print('Sábado ' + DateTime.saturday.toString());
            diaActivo = true;
          } else if (posicion == DateTime.sunday && alarmaInfo.domingo == 1) {
            print('Domingo ' + DateTime.sunday.toString());
            diaActivo = true;
          }
          if (diaActivo) {
            print(scheduledNotificationDateTime.weekday);
            await flutterLocalNotificationsPlugin.zonedSchedule(
                valorAlarma + scheduledNotificationDateTime.weekday, // id,
                alarmaInfo.titulo,
                '' + Utiles.diaSemana(scheduledNotificationDateTime.weekday),
                scheduledNotificationDateTime,
                const NotificationDetails(
                  android: AndroidNotificationDetails(
                    'Canal_ID',
          ...
          posicion = scheduledNotificationDateTime.weekday;
          print(posicion);
          print(scheduledNotificationDateTime.toString());
          print('****');
          */
